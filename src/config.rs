...
pub struct BuyHeuristic {
    ...
    pub growth_max_peg: f64,         // (#37) hard gate: reject a name whose PEG exceeds this — too expensive for the growth it actually delivers. Expressed as PEG (2.0 = "reject PEG > 2") so the config reads like the column; applied internally as its reciprocal, since the measurable field is `fund.peg_yield` = 1/PEG x 100 (peg_yield 100 <=> PEG 1, so the bar is 100/growth_max_peg). NOT the printed `peg` cell: that divides FMP's pe_ratio by the leg CAGR and pe_ratio is None in every backtest, so gating on it could never be validated. peg_yield is filled on BOTH paths from one fn (fetch.rs / backtest.rs -> core::peg_yield) -> train==serve, genuinely measurable. Consequence to know: the two numbers differ, so a row can PRINT PEG 1.9 and still be cut — the gate-review footer prints the gating number for exactly this reason. LOSS-MAKERS (eps_ttm <= 0) are also rejected: peg_yield is None there and "no earnings" is not a passing grade for a valuation ceiling. Genuinely absent fundamentals still pass, like every other data gate. Equity-only for free (crypto/ETFs carry no `fund`). 0 = off (default)
    ...
    pub growth_corr_cap: f64,        // (#41) REDUNDANCY skip on the shown stock table: walk the ranked rows and drop one whose 36-month trailing-return correlation with an already-kept row reaches this, refilling from below. Not a gate and not a score term — the rows it removes passed everything; it removes the SECOND copy of a bet, which is why it runs after the score trim rather than inside score_parts. Measured in the CORR-CAP probe, which is a WITHIN-RUN sweep and so immune to the (#40) cross-run universe noise: 0.8 is identical to off, 0.6 gives book +13.9->+14.3, excess +5.8->+6.2, win 86->88%, OOS +4.9/+6.8 -> +5.1/+7.2, and 0.4 is the same plateau. Needs >=12 overlapping months to judge a pair; an unjudgeable pair never blocks. 0 = off (default)
    pub growth_commodity_damp: f64,  // (#44) multiply the growth score of a COMMODITY-LINKED row — GICS Energy/Materials, or a fund whose name carries a commodity token (`picks::is_commodity`). Its CAGR tracks a mean-reverting input price, so the long record is a spot-price snapshot, not compounding: CF Industries ranked FIRST at +20%/yr on a −62% maxdd and R² 0.76, MPC eighth on −81% and 0.68. Rendered as `c` in the rank cell whether or not this knob is set. 1.0 = OFF (default, byte-identical to the pre-(#44) lane); ci-settings ships 0.8. BACKTEST-BLIND *by construction*: the pool carries no sector and no real fund names, so the damp is ×1.0 there — the validated edge is untouched AND this knob can never be swept, unlike every weight around it. Set by judgement; note the ci-settings receipt records that the nearest measurable cousin (an R²-steadiness damp) measured edge-NEGATIVE
...
impl Default for BuyHeuristic {
    fn default() -> Self {
        BuyHeuristic {
            ...
            growth_corr_cap: 0.0,          // (#41) off by default. 0.6 is the measured knee; 0.8 is indistinguishable from off
