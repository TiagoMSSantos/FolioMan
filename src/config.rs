...
pub struct Sizing {
    ...
    pub max_name_pct: f64,   // (P5) no single ISSUER may exceed this % of gross — `(#262)` made that true, by collapsing twin listings in `size::first_per_issuer` before the weights are struck; it read per-TICKER until then, and an 8% Alphabet book printed as two 4% rows. `(#264)` then took the number itself 6.0 -> 8.0, back to that same 8% high-water mark now that it is disclosed and per-issuer. Excess is redistributed to the UNCLAMPED members of the same class in proportion to their vol-target weight, iterated to a fixpoint (capped at 5 passes — the loop is monotone, so a pass that moves nothing ends it). 0 = off
    ...
    #[serde(skip_serializing_if = "is_default_book")] // the default serialises nothing, so every `sizing_fp` journaled before (#317) still matches
...
impl Sizing {
    ...
    /// (#317) How many non-coin names the equal-weight book buys: `book_names`, 0 read as 1, read here and nowhere
...
pub struct BuyHeuristic {
    ...
    pub min_avg_turnover_eur_stock: f64, // (#247) growth GATE (stocks): the equity lane's OWN turnover floor, same units as min_avg_turnover_eur. 0.0 = INHERIT that number and NOT "off" — this is a MIN bar, so 0-as-off would make the DEFAULT a relaxation, which non-negotiable #1 forbids; growth_min_cagr_etf documents the identical trap one class over. WHY THE CLASSES NEED DIFFERENT NUMBERS: min_avg_turnover_eur was only ever calibrated on FUNDS. It shipped at 100K (2026-07-02) because 100K killed genuine junk (Expat Romania €11K/day, Expat Greece €0, GraniteShares FAANG €0), then was raised to 500K (2026-07-03) for one stated purpose — dropping four thin ETF SIDE-LISTINGS (XSLE.DE €327K, CHIP.SW €266K, WTEC.SW €164K, XSLR.MI €125K). No stock was ever in that argument, yet `prefer_eu_listing` moves 266 US constituents onto their .DE lines and the bar is then read on Xetra turnover: (#232) measured liquidity failing 183 stocks with the swap on against 17 off, 10.8x. ONE NUMBER CANNOT SERVE BOTH: KLA.DE sits at €436K but MPN.DE is below the €250K near-miss boundary, i.e. UNDER the ETF side-listings at €266K/€327K, so any single floor admitting both stocks re-admits the funds the raise exists to kill. Resolved ONCE by `picks::turnover_floor` (non-negotiable #4) — the floor was three hand-copied spellings before. THE [FOIL] on-sale lane (`buy_score`) DELIBERATELY KEEPS READING THE GLOBAL KNOB: non-negotiable #6 forbids improving that lane. BACKTEST-BLIND, and verified rather than asserted — `core::backtest_quote` sets avg_turnover_eur to a 1e15 sentinel, so no realistic floor is reachable there and no golden moves; (#192) shipped exactly this claim FALSELY once, so the golden check is the proof, not the prose. Full receipt: the (#247) block in tests/ci-settings.yaml
    ...
    pub long_trend_cap: f64,         // cap on that long-leg CAGR (%/yr) fed into the reward, in BOTH lanes (growth `trend` and the on-sale foil's `long_reward`) — a +50%/yr coin doesn't 5× a +10%/yr one. 0 = OFF (uncapped); `tests/ci-settings.yaml` SHIPS 30.0 — (#3k) restored it and (#270) corrected the comments that still said otherwise. The readers go through `capped_trend`, which needs that guard because `min(cagr, 0.0)` would otherwise zero every positive CAGR
    ...
    pub peg_cagr_years: u32,         // (#331) GROWTH: the window the PEG DENOMINATOR is measured over, INDEPENDENT of the ranking leg. 0 = inherit `fixed_cagr_years` (default, byte-identical to today). WHY IT EXISTS: `growth_max_peg` divides by whichever rung the AGE ladder picked, so an old name is priced on a rate it may have stopped delivering a decade ago — live 2026-08-06, AMZN (29.2y, 20Y rung +23.4%/yr, its last 5y +4.7%/yr) printed PEG 1.63 and was ADMITTED at the 1.6 ceiling while VRT (8.0y, +47.9%/yr, six times the current growth) printed 1.70 and was REFUSED. The (#51) receipt: "That is not a valuation number, it is an age number." `fixed_cagr_years` also fixes it and was REFUSED at 5, 8 and 10, because it moves SEVEN readers at once (gate, trend, accel, sharpe, calmar, trend_health, LEG) and the damage landed in trend/accel at rank 1, dose-response monotone in how much the pin shortens an old name's leg. This knob moves the PEG denominator ALONE, which is the surgical fix (#51) and (#266) each name as the only surviving version of the idea. Read through `picks::peg_cagr_pct` — the ONE definition every PEG divides by (non-negotiable #4), equity and ETF alike. LEGAL VALUES ARE ONLY core::HORIZONS {1,2,5,8,10,20}; anything else misses the `perf_pct("{N}Y")` lookup, falls back to the age ladder, and the knob SILENTLY DOES NOTHING. A name with no leg at the pinned window falls back too, so this never rejects anything by itself. Edge-affecting -> validate `backtest {20,12,8} universe fund pit` before flipping, and the `fund` lane is MANDATORY there (`fund_lane_on` gates the as-of peg_yield fill, so on a price-only or `stress` run every PEG is None and this knob is inert by construction)
    ...
    pub growth_min_cagr_etf: f64,    // (#192) growth GATE (ETFs): the fund lane's OWN CAGR floor, same units and same two legs as growth_min_cagr. 0.0 = INHERIT growth_min_cagr and NOT "off" — this is a MIN bar, so a 0 floor would admit everything, which is a relaxation rather than the neutral default non-negotiable #1 requires; the `_etf` knobs that mean "off" at 0 (growth_max_peg_etf, growth_min_aum_etf) are all MAX bars, where 0 really is neutral. SEPARATE FROM THE EQUITY NUMBER BY THE SAME ARGUMENT growth_max_peg_etf ALREADY SHIPS ON, one gate over: a diversified basket cannot clear an equity-grade bar. Measured live 2026-09-01 — every ETF that scores is a SECTOR fund (IITU +25%/yr, VVSM +35%, SEMI +30%) while the broad S&P 500 accumulators score 0.0 at +15%/yr (VUAA.DE/SPYL.DE/AUM5.DE, each carrying a real 10y record via history_proxy), i.e. the canonical 20-year buy-and-hold instrument is refused by a bar fitted on single stocks. Structural precedent is growth_min_cagr_crypto, the class floor already branched at every site this one reads; the floor for all three classes is resolved ONCE by `picks::min_cagr_floor` (non-negotiable #4) rather than re-spelled per read site, which is what it was before. IT IS BACKTEST-GRADEABLE, and an earlier draft of this comment claimed the exact opposite ("the graded pool is S&P 500 constituents and holds no ETFs"): `class_census` (backtest.rs) puts the pool at ~4311 ETFs of 4954 and names that same sentence as the rotted receipt `sharpe_cap_etf` once shipped on, and arming this knob moves five FROZEN goldens. GRADED AND REFUSED at 15.0 (2026-09-01, `backtest universe fund pit`, Ship Rule v2 on the (#139) pit lane): every GUARD passes, and PRIMARY falls at BOTH graded horizons — 12y median +3.9 -> +3.3, 8y +5.3 (+5.6) -> +4.8 (+4.3). Same direction at both, so not a TIE, an outright refusal. Mechanism, from the disjoint rank slices: rank 1 is IDENTICAL (no fund ever reaches it) while the admitted funds land in ranks 6-10, and an equal-weight top-10 pays 1/N for each. So this ships at the 0.0 sentinel and the knob exists for the #4 de-duplication and for a pinned ETF-only scope (`growth_gate_reachability_pin` reads etf LIVE, crypto/stock INERT), NOT because a value was found. Full numbers and the re-arming rule: the (#192) receipt in tests/ci-settings.yaml
    pub growth_sector_leaders: usize, // (#328) growth GATE (stocks only): a SECOND way past `growth_min_cagr`, and the first one that is not a single global number. K > 0 = a stock also clears the CAGR floor when it is among the top K compounders IN ITS OWN GICS SECTOR and its long CAGR still beats the benchmark's over the same leg. WHY: `growth_min_cagr: 19.0` is one absolute bar across every sector, so it admits whichever sector actually compounded at 19%/yr — since ~2016 that is US tech, nearly alone. Measured on the live book 2026-09-19: 21 non-coin names for a 25-wide book, ~15 of them one bet (three Nasdaq-100 wrappers, four S&P-US-Tech wrappers, two semiconductor funds, WTEC/XAID, and 4 of the 5 stocks are semis) — so `max_sector_pct: 25.0` then refuses most of what the screen ranked and the remainder spills to index legs ((#296)/(#308)). Width is NOT the constraint (`top_picks`/`book_names` are 25 against 21 supplied), which is why this round moves the admission bar instead. A SECOND DOOR, OR'd: `growth_min_cagr` is untouched and this can only LOWER a floor, never raise one, so it can only ADD names and never evict a held one — "no worse" is structural and any refusal is attributable to the admits alone. STOCKS ONLY, enforced structurally in `picks::min_cagr_floor`'s equity branch: the door reads `Quote.sector`, which is None for ETFs and crypto by construction, and the backtest census (ETF 0/14 graded samples at 20y against stock 380/4619) means a fund-facing arm could not be graded at all. THE BACKSTOP IS THE BENCHMARK, not a new constant: the ruler is excess vs the index, so a name compounding below the index cannot add excess. Note the direction is OPPOSITE to (#326)'s regime valve, which capped a floor BY the index and was refused for admitting falling knives — here the index is a LOWER bound. Both constants were fixed before any number existed and are not to be tuned afterwards: K = 1 ("one leader per sector", the minimum non-arbitrary choice, ~11 sectors inside a 25-wide book). POINT-IN-TIME: the cohort map (`sector_floor`) is built in the backtest from the PREVIOUS cutoff's samples, never this one's. MEASURED AND REFUSED 2026-09-21 (`backtest {20,12,8} universe fund pit`, SECTOR BOOK row, windows scored 1112/2771/6175): the door holds its pre-registered `guard_holds` bar ONLY at 20y (n=22, excess +0.7 -> +1.0, worst identical at -10.0, both OOS halves up) and fails at 12y (n=33, +3.5 -> +3.2) and 8y (n=33, +4.6 -> +4.2) against a 0.1 tolerance. THE MECHANISM IS THE POINT: the WIN RATE RISES AT EVERY HORIZON (57->62, 75->81, 84->89) while mean excess falls at two — the door buys names that beat the index more OFTEN and by LESS, and in an equal-weight book each admit costs a 1/N slice of the leaders it displaces. Breadth, not return, which is (#317)'s and (#319)'s verdict a third time. So this ships at 0 and what actually ships is the MEASUREMENT: `stamp_sector_cohorts` writes each quote's point-in-time sector floor whether or not the knob is armed, so the SECTOR BOOK row prices this door at EVERY run for free by re-scoring with the knob flipped — the (#324) notch-shadow idea applied to a refusal. Do NOT retune K or the backstop to rescue it: both were fixed before the run, and a different value is a new round with its own pre-registered bar. Most plausible reopen is a ruler that does not pay 1/N ((#313) `equal_weight_book`, or a cap-weighted book), since dilution IS the refusal. 0 = off, byte-identical (proved: re-blessing the seven goldens produced +20 lines and MINUS ZERO) — and the code default, so the config-less CI job keeps its regime ((#171)). Full receipt: the (#328) block in tests/ci-settings.yaml
    pub growth_trend_weight: f64,    // growth SCORE: reward per %/yr of the long-leg CAGR (capped at long_trend_cap when that cap is on; 0 there = uncapped — the shipped lane caps at 30.0, (#270))
    ...
    pub growth_corr_cap: f64,        // (#41) REDUNDANCY skip on the shown stock table: walk the ranked rows and drop one whose 36-month trailing-return correlation with an already-kept row reaches this, refilling from below WHERE THERE IS ANYTHING TO REFILL FROM — measured live 2026-07-28 there is not: the table keeps 38/38 rows off, 37 at 0.8, 35 at 0.6 and 29 at 0.4, because `decorrelate_keep` stops at `n` and the live pool of ~38 intra-correlated US large-caps runs out of uncorrelated candidates first. The probe fills its book from a much deeper list, which is why its number does not transfer. See the (#41) receipt in ci-settings.yaml; it ships 0 = off for that reason. Not a gate and not a score term — the rows it removes passed everything; it removes the SECOND copy of a bet, which is why it runs after the score trim rather than inside score_parts. Measured in the CORR-CAP probe, which is a WITHIN-RUN sweep and so immune to the (#40) cross-run universe noise, and which shares `core::decorrelate_keep` with the live skip so probe and served rule cannot drift. TWO runs, agreeing on the plateau and disagreeing on which end of it leads: 0.8 is identical to off on both; the earlier run gave 0.6 book +13.9->+14.3, excess +5.8->+6.2, win 86->88%, OOS +4.9/+6.8 -> +5.1/+7.2 with 0.4 "the same plateau"; the 2026-07-28 run gives 0.6 book +14.3 OOS +5.8/+6.6 (late half BELOW off's +6.7) and 0.4 book +14.4, excess +6.3, worst flat, OOS +5.9/+6.8 — only 0.4 clears the probe's own ship rule there. 0.4 is therefore the value this knob WOULD take if it took one — ci-settings ships 0.0, for the live-pool reason stated above, and the two halves of this sentence used to contradict each other. (#140) NO LONGER SERVED-ONLY: since 2026-08-25 the same skip also runs in the GRADED path (`report_vs_benchmark` -> `corr_cap_rung` -> `core::decorrelate_keep`, the two-site shape `growth_value_floor_pct` uses), so THIS FIELD'S OWN VALUE is gradeable for the first time. The CORR-CAP probe had always swept the axis, but on its own ladder — it never read this field, so arming the knob in ci-settings moved the screen and left every verdict number byte-identical: a vacuous discharge, not a verdict. The graded twin runs PER LADDER RUNG (`n` = the rung being built, matching what the live site and the probe each pass), NOT on the cohort — a correlation cap's answer depends on how many names you are buying, whereas the (#75) value brake trims the cohort because a percentile is a statement about the pool. It deliberately does NOT reach `rank_slice_stats`, so the rank-1 h2h GUARD is measured on the same cohort armed or off and CANNOT move for this knob — read a passing h2h here as vacuous, never as evidence. Needs >=12 overlapping months to judge a pair; an unjudgeable pair never blocks. 0 = off (default)
    ...
    pub growth_value_weight: f64,    // (Item 20) authority of the P/E multiplier (value_factor) in the GROWTH lane only — BACKTEST-BLIND until (#147) filled `quote.pe_ratio` on `fund` runs; it is gradeable now, and (#147) graded it: 1.0 = full ×0.5..1.5 swing (DEFAULT, unchanged), 0.0 = neutral (off). The validated edge was measured with this term OFF, and (#1) zeroed it because pe_ratio was None in the backtest — (#147) removed that blindness and MEASURED the whole 0.25..1.0 ladder: it never reaches the argmax, so 0 stands on evidence rather than on ignorance. Defaulted so an older settings.yaml is unchanged.
    ...
    pub flat_run_max_years: f64,     // (#263) plateau trimmer, the splice trimmer's twin: drop history BEFORE the last run of byte-identical consecutive closes spanning this many years or more. A thin secondary listing's early years are often a single PLACEHOLDER price repeated for years, and that fiction then feeds life_cagr/MAXDD/R² exactly as a splice does — measured on the shipped long cache 2026-09-07, NVD.DE repeats 13.0750 for 71 monthly bars (5.9y) and KLA.DE repeats 3.267 for 104 (8.6y), against a longest CLEAN run of 8 bars (ABEA.DE/AMZ.DE, the 2008-09 GFC window) and 1 (ABEC.DE/IITU.L). TWO-SIDED, which is why it is a data fix and not an admission knob: the flat decade REFUSES NVIDIA (`cagr-life` 13.9%/yr vs a 19.0 floor, its sole blocker) and simultaneously FLATTERS KLA (an 8Y rung of +3962% off a placeholder). Expressed in YEARS, never bars — a bar count would inherit exactly the granularity defect `(#117)` measured and refused, since the cache holds 1mo/1wk/1d/1h/3mo/1y series side by side. Equality is EXACT (a placeholder repeats to the last bit; a real close does not) and a TRAILING plateau is never trimmed. Applied at the same three sites as the splice trim and composed with it by `max`. 0.0 = off (DEFAULT, byte-identical); ci-settings ships the measured 2.0. BACKTEST-BLIND in the same sense the splice knob records: it changes the INPUT universe, so before/after edges are two different measurements, not an A/B
    ...
    pub growth_record_weight: f64, // (#133) additive PENALTY per MISSING year of price record, docked from `base` ahead of every damp: `-weight * max(0, growth_record_full_years - long_years)`, where `long_years` is the leg `long_leg_fixed` actually found. Exists for one measured reason, from the `growth_min_leg_years` receipt: a short record COLLECTS SCORE FOR THE ABSENCE OF MEASURED BAD NEWS — its above-MA reads 0% and its maxdd is shallow because the drawdown has not happened yet, so the risk brakes cannot dock what they cannot see. That receipt refused the 2Y rung twice and pre-registered this term as the only condition for re-proposing it: "a record-length term in the score itself, not a post-hoc multiplier". `trust_factor` is the multiplier it rules insufficient. 0.0 = off, the term contributes exactly 0, and the DEFAULT
    pub growth_rank_normalise: f64, // (#144) authority of the CROSS-SECTIONAL RANK NORMALISATION of `score_parts`' ADDITIVE terms — the ones summing to `base`, eleven when this shipped and twelve since (#150) added `growth_base_offset`; that twelfth is CONSTANT across the pool, so the `sd == 0` guard passes it through untouched and this knob's authority is unchanged by it. Per scoring window, each term's LEVEL is replaced by its rank position mapped back onto that term's own pool distribution: `v' = v + w*(mean + sd*clip(inv_norm(rank_pct(v)), -3, 3) - v)`. 0.0 = off, `v' = v` for every term, and the DEFAULT — the shipped lane bit for bit. 1.0 = fully rank-Gaussianised. WHY IT IS UNIT-PRESERVING RATHER THAN A PLAIN z: `base` is a sum of terms in mutually incommensurable units (CAGR %/yr, accel pts, ROE %, yield %, trend_r2 0..1, underwater YEARS), and replacing a level with a z-score would silently invalidate every one of the 14 `WEIGHT_DIMS` values and every cap that has a receipt. Mapping back through the term's own mean/sd keeps the weights, the caps and their receipts meaning what they say, so the arm tests ONE thing: whether the ORDER improves once a single fat-tailed value can no longer take the argmax outright. The multiplicative damps (proximity/value/trust/overext/ter/commodity/fx/acc) are deliberately NOT normalised — they are bounded ratios, not levels, so they are not the failure mode this addresses. (#129) is why it exists: as the pool widens, Spearman(growth_score, peer-relative) goes +0.25 -> +0.03 at 8y and +0.10 -> -0.03 at 12y, rank 1 becomes the WORST disjoint slice at every horizon, and 87-99% of the ungated missed wealth files under `out-ranked` -- "A round on the score is the next honest move". The heuristic v2 spec is the form: "Rank->inverse-normal, not raw z-scores. Fundamental distributions are fat-tailed and a single outlier will otherwise dominate the composite."
    pub growth_base_offset: f64, // (#150) a CONSTANT added to `base`, the twelfth additive term, docked from nothing and ahead of every damp. It is NOT a tilt and ranks no name above another on its own — `score = base × M + liq_bonus`, so adding c contributes `c × M`, which ranks BY THE MULTIPLIER STACK (proximity, value, trust, overextension, TER, commodity, FX, share class) in proportion to c. That is the dial: how much of the order is decided by the additive terms' LEVELS versus by the multipliers, and nothing else in this file sets it. Exists because the shipped lane already has one by accident and nobody chose it — `growth_fund_extra`'s roic entry carries `neutral: 17.3` at `weight: 0.25`, so every row the factor does not cover collects a flat 4.325 inside `fund` (the (#59) mechanism), and the 12y ablation prints `fund_extra:roic` and a NULL factor at IDENTICAL edge (+157.0) and IDENTICAL delta (-25.3): the constant IS the whole measured contribution. This knob makes that quantity nameable and sweepable instead of a side effect of a missing-data fill. Deliberately ADDITIVE-ON-TOP and never a replacement for that `neutral`, which would move shipped behaviour at the default. NOTE it is not purely a RANKING knob — a large enough offset lifts names over `growth_min_score` — so (#149)'s VACUOUS tag does not automatically silence it. 0.0 = the term contributes exactly 0, the lane every golden holds, and the DEFAULT
    pub growth_record_full_years: f64, // (#133) the record length that counts as COMPLETE; only the shortfall below it is penalised, so a name with a longer leg is never rewarded for it. 20.0 because that is the hold this tool claims to pick for and the top rung of `long_leg`'s ladder. UNFITTED judgement value that never fires while `growth_record_weight` is 0
    ...
    pub hold_min_aum_eur: f64, // (#267) the CORE admission AUM floor, in EUR, as a named number instead of the `1e9` literal buried in `core::hold_miss_leg_at` -- the LAST bare literal among the CORE admission bars, every other one (`hold_max_ter`, `hold_size_sleeve_ter`, `hold_allow_swap`, `hold_allow_unknown_ter`) having been named already. Naming it is byte-identical, and the floor is STILL 1e9 in BOTH regimes: tests/ci-settings.yaml ships the same number the Default does. MISSING AUM STILL PASSES: the leg reads `aum_shown()` inside `if let Some(a)`, which is (#224)'s fix for non-negotiable #5, and this knob does not touch it -- only a KNOWN sub-floor AUM refuses. The printed reject reason formats ITS FLOOR FROM THIS FIELD (non-negotiable #4), so the message can never quote a number the filter did not use -- (#102)'s rule when it named `hold_max_ter`. (#269) CORRECTS THIS COMMENT. It read "tests/ci-settings.yaml ships 5e8" and carried a paragraph opening "WHY IT MOVED, and it is a POLICY change taken BY DECISION rather than by evidence" -- both written while the 5e8 probe was merely PRE-REGISTERED. (#268) then RAN it and REFUSED it on its own stop rule: 5 admits but 3 evictions. THE FLOOR NEVER MOVED. A pre-registration is not a receipt, and this is the second such lie the practice left in this file -- see `hold_domicile_penalty` seven fields down, which claims a 0.20 that (#235) refused. WHAT THAT REFUSED PROBE ACTUALLY SHOWED, re-read sleeve-by-sleeve by (#269): every eviction landed in a sleeve ALREADY AT ITS CAP, and every clean admit in a sleeve with room. So the refused paragraph's own argument was RIGHT -- (#224)'s and (#226)'s "none of these is a NEW index family for its sleeve" test answers the contested slots and says nothing about the empty ones -- and it was refused because a single floor cannot tell the two cases apart. `hold_min_aum_eur_new_family` is that argument, shipped with the guard that makes it true. 1e9 = the literal that shipped, byte-identical, and the DEFAULT
    pub hold_size_sleeve_ter: f64, // (#202) TER ceiling, in percent, for a NEW eighth CORE sleeve holding world SMALL-CAP funds; 0.0 = the sleeve is OFF and "small" stays an ordinary `core::NARROW` disqualifier, which is today's behaviour byte-for-byte. Why a sleeve and not a looser `hold_max_ter`: (#197) measured the ≤3/sleeve cap and found the 22 rows it hides are 20 further WRAPPERS of an index already shown, and since the table sorts cheapest-TER-first every hidden row is by construction more expensive than what is displayed — so relaxing the shared cap buys more tickers and strictly worse funds. Small caps are the one long-horizon exposure the lane structurally cannot reach: a different slice of the market rather than a re-weighting of names MSCI World already holds, which is why value/quality/momentum are deliberately NOT in `core::SIZE`. It needs its own tier because a 0.35% small-cap fund placed in the developed sleeve loses the TER sort to IWDA at 0.20% and is never printed. MEASURED by (#201): on a live 4594-ETF pond "small" refuses 29 geographically-broad funds and the TER leg refuses 163, which is the pre-registered rule this knob was built against. 0.0 = OFF, and the DEFAULT
    pub hold_ucits_or_domicile: bool, // (#102) accept an EU-domiciled fund as UCITS even when the NAME omits the token. `hold_miss_reason` requires the literal "ucits" in the fund name, and real cores do not always carry it — "ISHARES III PLC ISHRS CORE MSCI" is an Irish UCITS fund whose feed name has no such token, so it is rejected for a naming accident rather than a fact. true = the leg passes on the name token OR a `domicile` prefix outside `core::NON_EU`, the same blocklist `firds_etf_isins` already screens the FIRDS dumps with (one list, two readers). Names WITH the token are unaffected, and `domicile: None` (watchlist rows) still falls back to the name — missing data cannot newly pass a gate it used to fail. false = name token only, and the config-less DEFAULT. MEASURED 2026-09-01 by (#196) and SHIPPED TRUE in tests/ci-settings.yaml (same Default/shipped split as `print_h2h_mid_ladder`): on a live 5427-quote pond ON admits EXACTLY ONE fund and removes none — CSUS.SW, "iShares VII PLC - iShares MSCI USA ETF USD Acc", DOM IE, Acc, Full, €4.3B, TER 0.07%, 16.6y — a canonical MSCI USA UCITS tracker whose feed name carries no "ucits" token, which is the defect exactly. It was INVISIBLE until (#197) made `hold_per_tier` liftable: CSUS.SW lands in the US sleeve, which already had 14 qualifiers for 3 slots, so the CORE table was byte-identical and only the census line moved (US 14 -> 15). An admission knob measured on a capped table measures the cap
    pub hold_allow_swap: bool, // (#233) admit SWAP-BASED (synthetic) ETFs to the CORE lane. `hold_miss_leg`'s replication leg refuses everything that is not Full/Opt/Samp/Hybr, which means it refuses BOTH a fund BF labelled "Swap" AND a fund BF told us nothing about -- and `fetch.rs`'s `bf_row_meta` records `replicationMethod` present on only 70% of BF rows, with no name fallback of the kind `use_from_name` gives the share-class leg. So roughly three funds in ten died on a datum that was never fetched, which is non-negotiable #5 the same way (#224) found it on the AUM leg one leg later. true = the leg is INERT (admitting Swap and admitting None together exhaust the match); false = round 53's counterparty rule exactly as it shipped, and the DEFAULT. The other half of the trade is real and the repo already recorded it twice (`core::Quote::replication`'s comment, and the `repl` column note in tests/ci-settings.yaml): a swap fund's counterparty delivers the index total return GROSS, so it pays no US dividend withholding where an Irish physical fund pays 15% and a Luxembourg one 30%. THREADED into `core::hold_miss_leg_with` as an argument, never read there -- (#204)'s rule. REVERT: set false
    pub hold_allow_unknown_ter: bool, // (#240) admit a fund whose TER we never fetched to the CORE lane. `hold_miss_leg`'s TER leg refuses TWO populations under one funnel count -- funds that are genuinely too dear, and funds `ter_shown()` returns None for -- and the second is non-negotiable #5 ("missing data PASSES a gate") exactly as (#224) found it on the AUM leg and (#233) on the replication leg. This is the last big one. true = an absent TER passes the leg and the fund is judged on the remaining legs; false = today's behaviour byte-for-byte, and the DEFAULT. THE OVER-CAP ARM IS UNTOUCHED at both settings: a fund whose cost we DO know and which is over `hold_max_ter` still dies on leg 2, so this knob can never buy a dear fund -- ci-settings:4629 already refused that trade and this round does not reopen it. ADDITIVE BY CONSTRUCTION, and the codebase said so before the knob existed: `core::net_ter(None, ..)` returns the 9.9 "sort me last" sentinel, whose doc comment reads "it fires only if that leg is ever loosened" -- an unknown-TER fund therefore sorts BELOW every fund carrying a TER, so it can only fill a sleeve slot that would otherwise sit EMPTY. THREADED into `core::hold_miss_leg_with` as an argument, never read there -- (#204)'s rule, the same shape `hold_allow_swap` threads. REVERT: set false
    pub hold_min_aum_eur_new_family: f64, // (#269) a SECOND, LOWER CORE admission floor in EUR that buys NEW INDEX FAMILIES ONLY. A fund whose AUM sits between this and `hold_min_aum_eur` clears `core::hold_miss_leg_at`'s AUM leg, and then keeps its slot in `picks::hold_core_list` only if its `(sleeve, index family)` key is not already seated there by a fund clearing the STRICT floor -- so the relaxed floor can ADD a row and can never take one. 0.0 = OFF = `hold_min_aum_eur` is the only floor, the guard block never runs, and the lane is byte-identical (non-negotiable #1). WHY A SECOND FLOOR RATHER THAN A LOWER ONE: (#268) measured the single floor at 5e8 and refused it -- 5 admits, but 3 evictions, EVERY ONE of them in a sleeve already at its cap, where a cheap sub-floor wrapper of an index the sleeve ALREADY SHOWED displaced a EUR 20B incumbent. Splitting the floor answers exactly that. The sleeves with room still take their fund; the contested slots are never contested, because a fund that adds no family is not admitted to contest them. THE FAMILY KEY IS NOT A NEW NOTION: it is `core::sleeve_family_of` under `core::hold_breadth_tier`, the identical pair `core::family_first_order` keys on, shared through `picks::family_key` so the two sites cannot answer it differently (non-negotiable #4). MISSING AUM PASSES BOTH FLOORS ((#224), non-negotiable #5): only a KNOWN sub-floor AUM is ever guarded. WHY THIS CLEARS (#235)'s BAR for re-opening a refused floor -- "a tier-set chosen for a reason other than this run's ticker diff could ship; one chosen from it cannot" -- the guard is (#213)'s standing family-for-family standard, a rule that predates this run, not a shape read off its ticker diff. It is NOT a fix for the sort defect underneath: `picks::dom_rank` still buckets IE ahead of price in sleeves holding no US equity, and `hold_domicile_penalty` is still the built, measured, REFUSED answer to that
    pub hold_sector_sleeve_aum: f64, // (#273) a THIRD CORE admission AUM floor, scoped to the WORLD SECTOR SLEEVE rather than to a fund's index family. `core::aum_floor_for` applies it in place of `hold_min_aum_eur` for `SECTOR_TIER` alone, as a `min` so it can only ever LOWER the bar and only in that one sleeve. 0.0 = OFF = the base floor everywhere, byte-identical (non-negotiable #1). WHY IT IS NOT A LOOSENING IN DISGUISE: the EUR 1B base floor is calibrated on funds holding the WHOLE market, and a GICS sector is ~1/11th of it, so applying that number to a sector fund is a whole-market test on a slice, not a liquidity test. The value is DERIVED FROM THAT RATIO -- 1e9/11 is ~9.09e7, taken DOWN to a round 9e7 -- and not from what it admits, which is what keeps it clear of the argmax (#220)/(#224) forbid. WHY IT MAY BE RE-OPENED AT ALL: this is (#237) ARM 4, which was built, measured and then REMOVED on its own pre-registered ZERO-REMOVALS rule -- 2B7C.DE (iShares S&P 500 Industrials) entered the `industrial` family and displaced XDWI.L (Xtrackers MSCI World Industrials), because a sleeve fills cheapest-TER-first WITHIN a family. (#272) keys the family on the BENCHMARK INDEX, those two track different indices, and the shipped table prints BOTH -- so the displacement cannot recur, and the two arguments built on top of it ("no honest floor value avoids it", and the 14M-wide EUR 687M-701M argmax window) went with it. ARM 4's receipt closed "TO SHIP IT LATER it needs a DECISION, not a measurement"; the decision it named was whether +3 GICS families is worth XDWI.L becoming 2B7C.DE, and that trade no longer exists. MISSING AUM STILL PASSES ((#224), non-negotiable #5): the leg reads `aum_shown()` inside `if let Some(a)` and this knob does not touch it. STILL GUARDED BY (#269)'s FAMILY RULE: `picks::retain_new_family_only` filters on `clears_strict` and not on WHICH floor admitted a fund, so a sector admission below the strict floor still has to bring a family the sleeve does not already show
    pub hold_issuer_share_class_codes: bool, // (#274) read an ISSUER SHARE-CLASS CODE (`1C` = Xtrackers/DWS Capitalising) as an accumulating class when the fund's name carries no `acc`/`dist` word. false = OFF = a coded name stays an honest n/a and is refused at leg 4, byte-identical (non-negotiable #1). WHY IT NEEDS A KNOB AT ALL, when (#242)'s `dis` token did not: this arm ADMITS, and (#242)'s own receipt pre-registered the split -- "the share-class datum must be RESOLVED rather than assumed (the `dis` token is one such fix, issuer codes like Xtrackers' `1C`/`1D` are another and would ADMIT, so they need their own knob and their own argument)". A one-sided Dist arm can only ever refuse and needs no permission; an Acc arm can put a fund on the table and does. IT IS A RESOLUTION, NOT A LOOSENING: `1C` is not a guess about an unknown fund, it is the issuer's own name for the capitalising class, so this narrows the n/a set rather than waiving the leg the way (#242)'s refused `hold_allow_unknown_use` would have. REACHES ONLY FUNDS WITH NO BF ROW -- a fund whose BF payload names the share class never consults the name for an Acc, so the prize is bounded by that population and was unmeasured when this was written.
    pub hold_us_swap_ter_credit: f64, // (#233) TER credit, in percentage points, given to a SWAP fund at GEO tier 3 (US) when the CORE sleeve sorts -- the withholding it does not pay, expressed in the units the sort already speaks. 0.0 = no credit, byte-identical, and the DEFAULT. Tier 3 ONLY: a US index is 100% US dividends so the saving is unambiguous, where a world fund is ~65-72% US and pricing it needs a per-tier weight table this knob deliberately does not carry. It is a JUDGEMENT VALUE, NOT FITTED: 15% treaty rate x ~1.2% S&P 500 trailing yield is ~0.18, less the swap spread a synthetic fund pays its counterparty (0-10bp), so ~0.10. It is KNOWN to leave tier 3 unmoved on today's pond -- with `hold_family_first` on, AUM5.DE (Amundi S&P 500 Swap, 0.15%) can only take the `s&p 500` family slot by beating SPYL.DE at 0.03%, which needs a credit above 0.12 -- and a value chosen to clear that break would be reverse-engineered from the outcome, which is (#220)'s forbidden argmax. The mechanism is armed and starts paying the moment a swap fund lands within 0.10 of its family's best physical fund. THREADED into the comparator, never read inside it. REVERT: set 0.0
    ...
    pub hold_domicile_penalty: f64, // (#235) the US-dividend withholding an Irish fund does NOT dodge, in percentage points, added to the CORE sort's cost term so domicile and TER are compared in ONE unit. 0.0 = OFF = `picks::dom_rank`'s 0/1/2 bucket sorts AHEAD of price exactly as it shipped, which is the DEFAULT and byte-identical (non-negotiable #1); any value > 0 retires that bucket as a sort key and spends it as a cost instead. WHY: `Quote::domicile`'s own comment prices the edge at "~ +0.2%/yr on a US/world fund" -- a number in basis points that the bucket enforces as an INFINITY, so an IE fund at 0.24% beats a LU fund at 0.03% at any gap. (#233) then added `core::net_ter`, which prices the SAME effect in the SAME units, and said so in its own doc -- two representations of one number is what non-negotiable #4 exists to prevent. TIER-GATED, and the gate is DEFINITIONAL rather than fitted: the edge is a US treaty, so it is worth exactly zero in a sleeve whose index excludes US equity BY ITS OWN NAME (`core::NO_US_TIERS` -- emerging, ex-US, Europe, Japan, Asia-Pacific, single-country). THREADED into the comparator, never read inside it -- (#204)'s rule, as (#233) did for the swap credit. REVERT: set 0.0
    pub hold_factor_sleeve: bool, // (#217) turn on a TENTH CORE sleeve holding world FACTOR funds -- quality, value and equal weight, plus momentum since (#228) -- which `core::NARROW` otherwise refuses on the token alone even though their geography is already world-broad. false = those three stay ordinary NARROW disqualifiers, which is today's behaviour byte-for-byte and the DEFAULT. A plain bool and NOT a TER ceiling, unlike `hold_size_sleeve_ter`: the live pond has FOUR funds clearing the BASE 0.25% cap (2026-09-03 census), one per factor family, so the sleeve needs no ceiling of its own and there is none to walk upward -- (#228) pre-registered one for momentum and the census refused it, XDEM.SW clearing at 0.25% like the other three. IT OVERRIDES A SHIPPED PRINCIPLE BY DECISION, NOT BY EVIDENCE -- (#210)'s receipt says value/quality/momentum are "deliberately NOT in `core::SIZE`" because they re-weight names MSCI World already holds rather than adding an exposure. That judgement was never priced; this knob prices it. REVERT: set false, or revert on the (#210) principle alone -- no measurement is needed to justify turning it back off
    pub hold_sector_sleeve: bool, // (#218) turn on an ELEVENTH CORE sleeve holding world/US SECTOR funds -- technology, health care, financials, energy, industrials and the rest -- which `core::NARROW` refuses on the token alone even when the fund names a broad index. Ranked LAST, which is where a concentrated bet belongs in a ladder ordered broadest-first. THIS OVERRIDES `(#210)`'s shipped principle harder than `(#217)` did, and by DECISION rather than evidence: a factor fund re-weights a universe the lane already owns, whereas a sector fund holds a slice of it. Live probe 2026-09-02: nine funds clear every remaining leg at the BASE 0.25% cap across five families, and `(#214)`'s family-first fill is what spends the 3 slots on three DIFFERENT sectors instead of three technology wrappers. NO TER ceiling of its own and the receipt pre-registers against adding one. See `core::SECTOR` and the `(#218)` receipt in tests/ci-settings.yaml
    pub hold_family_first: bool, // (#214) fill each CORE sleeve FAMILY-FIRST: one pass takes the best fund of each DISTINCT index family (which `core::GEO` token matched), then the sleeve's own ranking resumes for the rest; false = today's pure (domicile, TER, AUM) order and the DEFAULT, byte-identical on every sleeve. Non-negotiable #1. The cap is documented as existing "so no index family crowds out the others" but it counts FUNDS, so a family holding the two cheapest funds in a sleeve takes two of its three slots. MEASURED 2026-09-02 on a live 4578-ETF pond, per sleeve: all-world 3f/3 shown, developed 2f/2, emerging 2f/2, US 2f/2, ex-US 1f/1, Europe 2f of 3f, Japan 1f/1, Asia-Pacific 1f/1 — so exactly ONE sleeve under-shows a family today and the knob moves exactly one row. Display-only and NOT gradeable, same as `hold_per_tier`: `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_family_key_benchmark: bool, // (#272) key a CORE fund's INDEX FAMILY on `Quote::benchmark` -- BF's own index name -- instead of on the `core::GEO` word its name happens to carry, falling back to that word whenever BF has no index name for the fund. ONE key, shared by `core::family_first_order`, `picks::retain_new_family_only` and the CORE census through `core::sleeve_family_at` (non-negotiable #4). WHY: the token is wrong in BOTH directions. It SPLITS one index in two -- `msci pacific` / `msci pac ` are the same MSCI Pacific ex Japan, which is exactly how (#269) seated HMXA.L next to CSPXJ.SW and had to record the leak -- and it MERGES two indices into one: MSCI World and MSCI World IMI both key `msci world`, MSCI ACWI and MSCI ACWI IMI both key `acwi`, and an IMI index is a different book (small caps in, ~99% of investable market cap against ~85%). NB `family_first_order`'s doc offers "FTSE Developed Europe" vs "... ex UK" as the merge example and that citation has ROTTED -- `ftse developed europe ex uk` is its own GEO token and the pair separates today; the two pairs above were verified against `sleeve_family_of` before this shipped. Both directions cost a row, and every cap read off that count under (#220) sat on it. (#237) printed the benchmark count beside the token count as the census's `Yb` column PRECISELY to ask whether this table should be a menu of EXPOSURES or of PRODUCTS without shipping an answer; the user took that decision on 2026-09-08 and this knob is it. NO NORMALIZER AND NONE IS WANTED: `fetch::bf_row_meta` lowercases, trims and maps empty to None at capture, and BF normalizes the string, so same-index funds share the literal -- which is why `Quote::benchmark`'s doc calls it usable "beyond exact `==`" by nothing else. MISSING BENCHMARK KEYS AS IT DOES TODAY (non-negotiable #5): absent data can never newly split a fund from its own family. Coverage was ~84% of the CORE-eligible pond at ship, so the lane runs on a MIXED key and the receipt says so. false = the GEO token is the key, byte-identical, and the DEFAULT (non-negotiable #1). Display-only and NOT gradeable, same as `hold_family_first`: `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_per_tier_all_world: usize, // (#207) rows kept in the ALL-WORLD sleeve alone, overriding `hold_per_tier` there; 0 = no override, inherit it, which is byte-identical to today on every tier and the DEFAULT. Tier 0 is the one sleeve where a further row is a DIFFERENT PRODUCT rather than another wrapper of an index already shown, which is the finding (#197) rested its cap of 3 on: ACWI (large+mid), ACWI IMI (large+mid+SMALL) and FTSE All-World track different universes, so ranking them against each other on TER alone hides an exposure behind a price. It became live-visible only when (#203)'s GEO census found `all country world` missing from `core::GEO` — a hyphen against a space — and admitting it put SPYY.DE (€14.8B, 0.12%) and SPYI.DE (ACWI IMI, €6.6B, 0.17%) into a sleeve already full at 3, where cheapest-first would have evicted SSAC.L and hidden the IMI fund entirely. Display-only and NOT gradeable, exactly as `hold_per_tier`: `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane. NOT `VERDICT_TOP`, NOT (#120)'s forbidden argmax — that is the GRADED basket; this is a table height
    pub hold_per_tier_developed: usize, // (#251) rows kept in the DEVELOPED sleeve alone (tier 1), overriding `hold_per_tier` there, AND the switch for the `global equity` arm in `core::geo_tier_at`: 0 = inherit the shared cap and the arm is INERT, which is byte-identical on every sleeve and the DEFAULT (non-negotiable #1); any N > 0 caps the sleeve at N and opens the arm. ONE knob rather than a bool plus a cap, the shape `hold_per_tier_country` (#227) and `hold_per_tier_nasdaq` (#239) already ship, and here the coupling is LOAD-BEARING rather than tidy: at the shared cap of 3 the token EVICTS instead of adding. `hold_family_first` takes the best fund of each distinct family in rank order, so a fourth family against three slots drops the LAST — VHVE.L, the only fund of the `ftse developed` family — and losing a whole exposure is what (#213)'s family-for-family standard refuses. The token is reachable only where a slot exists for it. 4 = the FAMILY COUNT read off the live census (msci world, prime global, ftse developed, + global equity), not an argmax. REVERT: set 0. Display-only and NOT gradeable, exactly as `hold_per_tier` and its seven elder twins — `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_per_tier_us: usize, // (#231) rows kept in the US sleeve alone, overriding `hold_per_tier` there; 0 = no override, inherit it, byte-identical to today on every tier and the DEFAULT. The FIFTH per-sleeve override, earned exactly as the four before it: (#231) gave GEO the `ftse north america` spelling it lacked, which takes tier 3 from THREE index families to FOUR against a shared cap of 3 — so the cap would hide a whole EXPOSURE rather than another wrapper of an index already shown, the invariant (#197) rested its cap on. Written against the LITERAL tier 3 in `core::tier_cap` because the geographic rungs have no named constant, unlike the four optional sleeves. 4 = the FAMILY COUNT read off the live census, not an argmax. REVERT: set 0. Display-only and NOT gradeable, exactly as `hold_per_tier` and its four elder twins — `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_per_tier_europe: usize, // (#237) rows kept in the EUROPE sleeve alone, overriding `hold_per_tier` there; 0 = no override, inherit it, byte-identical to today on every tier and the DEFAULT. The SIXTH per-sleeve override, earned exactly as the five before it: (#237) gave GEO the `ftse developed europe ex uk` spelling it lacked, which takes tier 5 from THREE index families to FOUR against a shared cap of 3 — so the cap would hide a whole EXPOSURE rather than another wrapper of an index already shown, the invariant (#197) rested its cap on. The UK is roughly a fifth of developed Europe, so ex-UK is an allocation decision and not another packaging of one index. Written against the LITERAL tier 5 in `core::tier_cap` because the geographic rungs have no named constant, unlike the four optional sleeves. 4 = the FAMILY COUNT read off the live 2026-09-04 census, not an argmax. REVERT: set 0, which also makes the GEO token inert — with the cap at 3 the fourth family cannot print. Display-only and NOT gradeable, exactly as `hold_per_tier` and its five elder twins — `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_per_tier_size: usize, // (#230) rows kept in the SMALL-CAP sleeve alone, overriding `hold_per_tier` there; 0 = no override, inherit it, byte-identical to today on every tier and the DEFAULT. The FOURTH per-sleeve override, earned the same way the other three were: (#229) opened the sleeve to every region and (#230) gave GEO the US small-cap spellings it lacked, which together take it from ONE index family to FOUR against a shared cap of 3 — so the cap would hide a whole EXPOSURE rather than another wrapper of an index already shown, the invariant (#197) rested its cap on. 4 = the FAMILY COUNT read off the 2026-09-04 live census (`small-cap 4 (4f)`), not an argmax: supply is exactly four funds, one per family (WSML.L msci world EUR 7.3B 0.35%, R2US.L russell 2000 EUR 4.6B 0.30%, XXSC.DE msci europe EUR 3.0B 0.30%, VGSA.DE ftse global small 0.22% AUM not served), so the sleeve prints at family-count equilibrium and there is no fifth fund a larger value could reach. REVERT: set 0. Display-only and NOT gradeable, exactly as `hold_per_tier` and its three elder twins — `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_per_tier_factor: usize, // (#228) rows kept in the world FACTOR sleeve alone, overriding `hold_per_tier` there; 0 = no override, inherit it, byte-identical to today on every tier and the DEFAULT. The THIRD per-sleeve override, and it exists for the reason `hold_per_tier_sector` does: adding "momentum" to `core::FACTOR` gives that sleeve a FOURTH index family against the shared cap of 3, so the cap would hide a whole EXPOSURE rather than another wrapper of an index already shown — the invariant (#197) rested its cap on. 4 = the FAMILY COUNT read off the 2026-09-03 census, not an argmax: world/developed factor supply is exactly four funds, one per family (MWEP.L equal weight EUR 1.6B 0.20%, XDEQ.DE quality EUR 2.8B 0.25%, XDEV.DE value EUR 3.7B 0.25%, XDEM.SW momentum EUR 1.9B 0.25%), so the sleeve prints at family-count equilibrium the moment the token lands and there is no fifth fund for a larger value to reach. REVERT: set 0. Display-only and NOT gradeable, exactly as `hold_per_tier` and its two elder twins — `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_per_tier_sector: usize, // (#220) rows kept in the world SECTOR sleeve alone, overriding `hold_per_tier` there; 0 = no override, inherit it, byte-identical to today on every tier and the DEFAULT. The SECOND per-sleeve override, and it exists because (#218) broke the invariant (#197) rested its shared cap of 3 on: "every sleeve prints at least as many rows as it has index FAMILIES, so a further slot buys only another WRAPPER of an index already shown". MEASURED 2026-09-02, live `screen`: the sector sleeve is the only one where that is false — supply 9 funds across 5 FAMILIES into 3 slots, so two whole exposures are hidden rather than two duplicates. Lifting the shared `hold_per_tier` instead would buy 12 wrappers to reach them, which is what (#197) refused. 5 = the FAMILY COUNT, not an argmax: the four funds beyond it are MSCI World re-wrappers of technology, health care, energy and financials, all three of which the sleeve already prints. REVERT: set 0. Display-only and NOT gradeable, exactly as `hold_per_tier` and `hold_per_tier_all_world` — `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_per_tier_country: usize, // (#227) turn on a TWELFTH CORE sleeve holding SINGLE-MARKET funds -- DAX, Euro Stoxx 50, MSCI EMU, FTSE 100, Korea, India and China -- and cap its rows, in one number: 0 = the sleeve is OFF and the lane has eleven sleeves exactly as it shipped, which is the DEFAULT and byte-identical (non-negotiable #1); any N > 0 opens it with at most N rows. ONE knob rather than a bool plus a cap, the shape `hold_size_sleeve_ter` already ships. THIS OVERRIDES `(#210)`'s shipped "broad market exposure only" principle harder than `(#217)` or `(#218)` did, and BY DECISION, NOT BY EVIDENCE: a factor fund re-weights a universe the lane already owns and a sector fund holds a slice of it, but a single-market fund is a concentrated country bet the lane has refused since it existed. The user took that decision explicitly, unrestricted, after being offered the narrower emerging-markets-only scope and declining it. MEASURED 2026-09-03, live `screen` census of 1549 geo-less, tilt-less EU-buyable ETFs against 27 candidate spellings: SEVEN reach a fund clearing all five admission legs, twenty reach none and are deliberately unspelled. NO TER ceiling of its own and none is needed -- all thirteen admitted funds price 0.09%-0.20% against the BASE 0.25% cap. HONEST LEDGER: India, China and Korea already sit inside the emerging sleeve at index weight, so a dedicated row is a deliberate overweight, and China's 20-year record is poor despite its GDP growth -- this table forecasts no returns and never has. REVERT: set 0, which restores the shipped table byte-for-byte. Display-only and NOT gradeable, exactly as `hold_per_tier` and its two elder twins -- `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk reaches this lane
    pub hold_per_tier_nasdaq: usize, // (#239) turn on a THIRTEENTH CORE sleeve holding the NASDAQ-100 -- the one member of the exchange's index suite that is a broad US large-cap holding rather than a bet on an industry -- and cap its rows, in one number. Same shape as `hold_per_tier_country`: 0 = the sleeve is OFF, `core::NARROW`'s `nasdaq` token refuses every such fund at leg 0, and the lane prints its twelve sleeves exactly as it shipped (non-negotiable #1). NOT A `GEO` TOKEN AND DELIBERATELY SO: (#234) pre-registered a US cap raise to seat iShares Russell 1000 GROWTH and REFUSED it, because `core::geo_tier`'s rule is GEOGRAPHY, NEVER INDUSTRY OR STYLE, and a financials-free top-100-by-cap on one exchange meets that objection exactly -- so this sleeve is the fifth use of the `geo_tier_at` escape hatch (#202)/(#217)/(#218)/(#227) opened, and `hold_per_tier_us` does not move. MEASURED 2026-09-05 on a live 4568-ETF pond: 66 funds carry the token, exactly FIVE clear every leg (NASD.L, EQQS.L, XNAS.L, ANX.PA, ANXU.L) and all five track the SAME index, so the family count -- (#220)'s rule, and the cap -- is 1. 0 = OFF, and the DEFAULT
    pub hold_per_tier: usize, // (#197) rows kept per breadth sleeve in the buy-and-hold CORE table, as a named number instead of the bare `const HOLD_PER_TIER` it replaces. The cap is what makes a naming accident INVISIBLE rather than merely costly: `hold_ucits_or_domicile`'s receipt notes that a fund deleted on spelling does not shorten the list, because a worse fund refills the slot — so the cap and the two admission knobs are one question, and this is its third arm. Read at every one of the four sites that held the const, INCLUDING the `≤N/sleeve` header text, so the printed cap cannot quote a number the filter did not use. Display-only and NOT gradeable, permanently: `core::hold_miss_reason` needs TER/AUM/replication/share-class, and `backtest::stamp_asset_class` fills only name/instrument_type/sector, so every backtest quote dies on its TER leg and no walk can reach this lane. NOT `VERDICT_TOP` and NOT (#120)'s forbidden argmax — that is the GRADED basket; this is a table height. 3 = today's literal, byte-identical, and the DEFAULT
    ...
    pub underwater_monthly: bool, // (#223) fill `underwater_yrs` on a MONTHLY-cadence backtest instead of leaving it None. `growth_underwater_weight` docks a name per year of its longest below-prior-peak stretch — drawdown DURATION, which the maxdd depth cap cannot see — and it fires on the LIVE screen (daily closes) every day. But `monthly = long || years >= 8` makes EVERY graded horizon monthly, and `longest_underwater_yrs` hardcoded /252, so `core::backtest_quote` blanked the stat on 8y, 12y and 20y alike: the term contributed exactly 0 to every name in every verdict and every golden the repo produces. `(#3k)` 2026-07-29 proved that empirically — zeroing the weight reproduced the 20y-stress grid metric-for-metric — and could only record it, because the fix is this knob. So the shipped 0.3 has NEVER been graded at the horizons it is meant to serve, while shaping the live list. true = the divisor becomes the run's own cadence and the monthly path fills, putting the term into the verdict. A DURATION converts by division, so there is no `vol_daily_equivalent`-style sqrt rescale; the honest ceiling is that monthly bars floor sub-month stretches to 0.0. false = the daily-only blank, today's behaviour, byte-identical, and the DEFAULT
    ...
    pub print_h2h_mid_ladder: bool, // (#135) print the same-window head-to-head a SECOND time against the 6-10 book, beside today's 11-20 one. WHY: the 11-20 comparison book only exists in a window holding >=11 names, and (#124) measured that the gates admit no more than ~10 — so Ship Rule v2's rank-1 h2h GUARD is starved by the very gate values it exists to revalidate. (#130) recorded the symptom and treated it as a fact of life ("the PIT universe exists and was DECLINED: the rank-1 h2h rests on 2/4/6 windows"), which is why the v2 task pack's P0b — re-sweeping the five lookahead-fitted knobs on the point-in-time pool — has never run. The 6-10 book needs only 6 names, so it survives that ceiling; printing BOTH from ONE run is what makes the two comparable without grid drift. The pair of DENOMINATORS is itself the measurement: hn(11-20) vs hn(6-10) reads the admission ceiling directly. Display only — no score, no gate and no journal field. (#160) IT IS NOW WHAT THE SHIP RULE READS at 12y and 8y (>= 50% AND not below control on this book); 20y still reads the 11-20 line. That is a grading-rule change recorded in tests/ci-settings.yaml, not a behaviour change here — this field still only prints. false = today's single h2h line, byte-identical, and the DEFAULT
    pub print_held_loser_factors: bool, // (#159) print, for the GROWTH lane, the held book split into the names that MADE money and those that LOST it, with each registered fund factor's median on both sides. WHY: every other instrument in the report prices what the ranking MISSED (`print_recall_capture`, `missed_winner_reasons`) or what the gates REJECTED (`gate_audit`, `gate_sweep`). Nothing priced what the lane BOUGHT AND LOST — and that is the only cohort a NEW gate can reach, since a gate cannot act on a name the book never held. Attribution has to be by FACTOR rather than by gate because a held name cleared every gate by construction, so `picks::gate_failures` is empty for all of them. The column that matters is the GAP between the two medians: a factor whose cohorts do not differ is a gate that would reject winners and losers together, which is what five refused rounds of literature-proposed gates kept discovering the expensive way. Display only — no score, no gate, no journal field. false = today's output, byte-identical, and the DEFAULT
...
impl Default for BuyHeuristic {
    fn default() -> Self {
        BuyHeuristic {
            ...
            min_avg_turnover_eur_stock: 0.0, // (#247) 0 = INHERIT min_avg_turnover_eur (see the field doc: on a MIN bar, 0 cannot mean "off"). Neutral by construction, so the lane is byte-identical to before this knob existed; tests/ci-settings.yaml ships the real 100000
            ...
            long_trend_cap: 30.0,          // cap the long-leg CAGR at 30%/yr (a +46%/yr coin doesn't dwarf a +14%/yr one). 0 = OFF (uncapped). (#270) CORRECTION: this comment used to say the live fixture ships 0 — it does NOT. (#3h) turned it off by direction on 2026-07-26; (#3k) RESTORED 30.0 on 2026-07-29 off the 20y-STRESS ladder (peak 30, +623.5) and that is what ships. Default and live now AGREE at 30; the default stays 30 per the house rule that defaults are neutral and ci-settings carries live, and because it keeps the "cap still clamps when enabled" test honest. CURVED at 12y (#3g): the ceiling wanted to come DOWN, not up — 15/20/25/30 read edge +175.6/+185.1/+191.1/+181.3 and everything above falls monotonically (40 +177.3, 50 +168.1, 60 +168.1), confirming the older on-horizon 30->50 point-test. OFF was then MEASURED on the same n=1338: edge +169.8, i.e. -11.5 vs 30 and -21.4 vs the 25 peak. Knob is SHARED with the on-sale foil (`long_trend_weight × capped_trend`), so a growth-lane curve prices only half its effect
            ...
            peg_cagr_years: 0,             // (#331) off: the PEG divides by the same leg the ranking does. Set 8 to price every name's PEG on its 8Y growth rate while the score keeps the age ladder
            ...
            growth_min_cagr_etf: 0.0,      // (#192) 0 = INHERIT growth_min_cagr (see the field doc: on a MIN bar, 0 cannot mean "off"). Neutral by construction — and tests/ci-settings.yaml ships the SAME 0.0, because 15.0 was graded and refused, so both regimes and every golden are byte-identical to before this knob existed
            growth_sector_leaders: 0,      // (#328) OFF, byte-identical — the door opens only where a config arms it, so the config-less CI job ((#171)) keeps today's regime
            ...
            growth_max_peg: 0.0,           // (#37) off by default. MEASURABLE (peg_yield is filled in the backtest) -> validate on `backtest universe fund pit` before shipping a value. (#266) ci-settings ships 1.6 and the 2.0 this line used to suggest is MEASURED WORSE: the {1.75, 2.0, 2.5} ladder fails PRIMARY monotonically at 12y and 8y. Read that receipt before proposing a value here
            ...
            growth_value_weight: 1.0,      // (Item 20) FULL P/E-multiplier authority by default (=current behavior, no change). The shipped config sets 0; (#147) un-blinded this term in the backtest and measured it there — see its receipt for why 0 stands
            ...
            flat_run_max_years: 0.0,       // (#263) OFF by default (older settings.yaml unchanged); ci-settings ships the measured 2.0 — the valley between the longest CLEAN flat run (8 monthly bars = 0.67y, the 2008-09 GFC window on Xetra) and the shortest corrupt one (71 bars = 5.9y, NVD.DE)
            ...
            growth_record_weight: 0.0, // (#133) 0 = the term contributes exactly 0.0 — the lane every golden holds.
            ...
            hold_min_aum_eur: 1e9, // (#267) the literal that shipped inside `core::hold_miss_leg_at`, extracted verbatim. (#269): tests/ci-settings.yaml ships THE SAME 1e9. This line read "This is the CONFIG-LESS shape only: tests/ci-settings.yaml ships 5e8" -- written while that probe was pre-registered, and left standing when (#268) refused it.
            hold_min_aum_eur_new_family: 0.0, // (#269) OFF = `hold_min_aum_eur` is the only floor, `picks::hold_core_list`'s guard block is skipped whole, and the lane is byte-identical. Non-negotiable #1. tests/ci-settings.yaml ships 5e8.
            hold_sector_sleeve_aum: 0.0, // (#273) OFF = `core::aum_floor_for` returns the base floor for every tier including SECTOR_TIER, and the lane is byte-identical. Non-negotiable #1. tests/ci-settings.yaml ships 90000000.
            hold_issuer_share_class_codes: false, // (#274) OFF = `fetch::share_class_code` returns None for every name and the share-class resolver answers exactly as it always has. Non-negotiable #1.
            hold_size_sleeve_ter: 0.0, // (#202) OFF — "small" is an ordinary NARROW disqualifier and the lane has seven geographic sleeves, exactly as it shipped.
            hold_ucits_or_domicile: false, // (#102) OFF = the name token is the only way to prove UCITS-ness. This is the CONFIG-LESS shape only: tests/ci-settings.yaml ships this `true` since (#196) measured it admitting one real core (CSUS.SW) and removing nothing.
            hold_allow_swap: false, // (#233) OFF = swap-based funds AND funds with no replication datum are both refused, round 53's rule exactly as it shipped. Non-negotiable #1. tests/ci-settings.yaml ships this `true`, the same Default/shipped split as `hold_ucits_or_domicile` and `print_h2h_mid_ladder`.
            hold_allow_unknown_ter: false, // (#240) OFF = a fund with no TER datum dies on leg 2 exactly as it shipped. Non-negotiable #1. tests/ci-settings.yaml ships this `true`, the same Default/shipped split as `hold_allow_swap` and `hold_ucits_or_domicile`.
            hold_us_swap_ter_credit: 0.0, // (#233) 0.0 = the sleeve sorts on raw TER and `dom_rank` reads the real domicile, exactly as it shipped. Non-negotiable #1. Dead on its own: with `hold_allow_swap` off no swap fund reaches the list for it to credit.
            hold_name_tokens: false, // (#102) OFF = bare `contains`, today's behaviour, mid-word accidents included. (#195) MEASURED it at ZERO on a live 5427-quote pond — supply identical sleeve-for-sleeve, same 20 CORE rows — and kept it OFF: ON also tightens the GEO table, so it can REJECT a name whose only geography token sat mid-word, and today's pond exercises none of that path.
            hold_domicile_penalty: 0.0, // (#235) OFF = `dom_rank`'s bucket is still the sort key ahead of price, exactly as it shipped. Non-negotiable #1. (#269): tests/ci-settings.yaml ships THIS SAME 0.0 -- there is no Default/shipped split here, the knob is off in both regimes. This line read "ships this 0.20, the same Default/shipped split as `hold_allow_swap` and `hold_us_swap_ter_credit`", written while that arm was pre-registered and left standing when (#235)'s own receipt REFUSED it on the emerging sleeve.
            hold_factor_sleeve: false, // (#217) OFF -- "quality"/"value"/"equal weight" are ordinary NARROW disqualifiers and the lane has nine sleeves, exactly as it shipped. Non-negotiable #1.
            hold_sector_sleeve: false, // (#218) OFF -- every sector token stays an ordinary NARROW disqualifier and the lane has ten sleeves, exactly as it shipped. Non-negotiable #1.
            hold_family_first: false, // (#214) OFF — the sleeve fills in pure (domicile, TER, AUM) order, exactly as it shipped. Non-negotiable #1.
            hold_family_key_benchmark: false, // (#272) OFF = the `core::GEO` token is the family key, exactly as it shipped, and `core::sleeve_family_at` returns `sleeve_family_of` unchanged. Non-negotiable #1. tests/ci-settings.yaml ships true.
            hold_per_tier_all_world: 0, // (#207) 0 = inherit `hold_per_tier`, i.e. today's behaviour on every sleeve including tier 0. Non-negotiable #1.
            hold_per_tier_developed: 0, // (#251) 0 = inherit `hold_per_tier` AND the `global equity` arm stays inert, i.e. today's behaviour on every sleeve including tier 1. Non-negotiable #1.
            hold_per_tier_us: 0, // (#231) 0 = inherit `hold_per_tier`, i.e. today's behaviour on every sleeve including tier 3. Non-negotiable #1.
            hold_per_tier_europe: 0, // (#237) 0 = inherit `hold_per_tier`, i.e. today's behaviour on every sleeve including tier 5. Non-negotiable #1.
            hold_per_tier_size: 0, // (#230) 0 = inherit `hold_per_tier`, i.e. today's behaviour on every sleeve. Non-negotiable #1.
            hold_per_tier_factor: 0, // (#228) 0 = inherit `hold_per_tier`, i.e. today's behaviour on every sleeve. Non-negotiable #1.
            hold_per_tier_sector: 0, // (#220) 0 = inherit `hold_per_tier`, i.e. today's behaviour on every sleeve
            hold_per_tier_nasdaq: 0, // (#239) 0 = the Nasdaq-100 sleeve is OFF and the lane has twelve sleeves, exactly as it shipped. Non-negotiable #1.
            hold_per_tier_country: 0, // (#227) 0 = the country sleeve is OFF and the lane has eleven sleeves, exactly as it shipped. Non-negotiable #1.
            hold_per_tier: 3, // (#197) the literal `HOLD_PER_TIER` shipped, extracted verbatim — moving it changes how many CORE rows print and nothing else.
            ...
            underwater_monthly: false, // (#223) OFF = the daily-only blank every golden and every monthly receipt in ci-settings was measured under, so `underwater_yrs` stays None at 8y/12y/20y and `growth_underwater_weight` contributes 0 there exactly as it has. Turning it on makes a SCORE term newly live at every graded horizon — it moves the ranking, not the pool — so it must be graded arm-vs-control before it ships, and it cannot be read off any pre-(#223) grid
            ...
            print_held_loser_factors: false, // (#159) OFF = the report without the held-loser split. tests/ci-settings.yaml ships this `true` and the fixture pins that file, so the blessed goldens carry the ON output; this Default is the config-less shape only.
...
pub fn flat_run_max_years() -> f64 {
    use std::sync::OnceLock;
    static YEARS: OnceLock<f64> = OnceLock::new();
    *YEARS.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.flat_run_max_years)
            .unwrap_or(0.0)
    })
}
...
/// (#267) see [`hold_max_ter`], whose shape this copies exactly. The printed reject reason formats this
/// same number, so the floor and the message it quotes cannot drift apart (non-negotiable #4).
pub fn hold_min_aum_eur() -> f64 {
    use std::sync::OnceLock;
    static FLOOR: OnceLock<f64> = OnceLock::new();
    *FLOOR.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_min_aum_eur)
            .unwrap_or(1e9)
    })
}

/// (#269) The relaxed CORE AUM floor that buys NEW INDEX FAMILIES ONLY. Copies [`hold_min_aum_eur`]'s
/// shape exactly, and means nothing on its own: `core::hold_miss_leg_at` admits down to it, and
/// `picks::hold_core_list` is where the "and only for a family the sleeve does not already show"
/// half is enforced, because the leg is per-fund and cannot see what else the sleeve holds.
///
...
pub fn hold_min_aum_eur_new_family() -> f64 {
    ...
    *FLOOR.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_min_aum_eur_new_family)
            .unwrap_or(0.0)
    })
}

/// (#273) The SECTOR SLEEVE's own CORE AUM floor. Copies [`hold_min_aum_eur_new_family`]'s shape,
/// and like it means nothing on its own: `core::aum_floor_for` decides where it applies (SECTOR_TIER
...
pub fn hold_sector_sleeve_aum() -> f64 {
    ...
    *FLOOR.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_sector_sleeve_aum)
            .unwrap_or(0.0)
    })
}

/// (#274) Whether an issuer share-class code may resolve a name-silent fund to Acc.
///
/// `#[mutants::skip]` for the reason [`hold_family_first`] and [`hold_family_key_benchmark`] carry
/// it: this accessor holds NO decision, it only reads a bool out of the merged config, and the gate
/// cannot tell a forced `false` here from the shipped default. The decision this knob buys lives in
/// `fetch::share_class_code`, which is graded directly by its own tests.
#[mutants::skip]
pub fn hold_issuer_share_class_codes() -> bool {
    use std::sync::OnceLock;
    static ON: OnceLock<bool> = OnceLock::new();
    *ON.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_issuer_share_class_codes)
            .unwrap_or(false)
    })
}
...
pub fn hold_size_sleeve_ter() -> f64 {
    ...
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_size_sleeve_ter)
            .unwrap_or(0.0)
    })
}

/// (#197) see [`hold_name_tokens`] for why these are free accessors. `hold_core_list` and
/// `print_hold_core` take `&[Quote]` and no `tuning`, and the header line quotes this same number,
/// so routing both through one reader is what stops the printed cap and the applied cap drifting.
...
pub fn hold_per_tier_all_world() -> usize {
    ...
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_all_world)
            .unwrap_or(0)
    })
}

/// (#251) the DEVELOPED sleeve's own row cap, and the switch for the `global equity` arm; 0 = inherit
/// [`hold_per_tier`] and leave that arm inert. Same `#[mutants::skip]` as its seven twins and for the
/// same reason: the value is a process-wide `OnceLock` no test can flip, so every branch reading it
/// off-default is one the mutation gate cannot reach. The cap is THREADED as an argument into
/// [`crate::core::tier_cap`] and [`crate::core::geo_tier_at`], and graded there instead.
#[mutants::skip]
pub fn hold_per_tier_developed() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_developed)
            .unwrap_or(0)
    })
}

/// (#231) the US sleeve's own row cap; 0 = inherit [`hold_per_tier`]. Same `#[mutants::skip]` as its
/// four twins and for the same reason: the value is a process-wide `OnceLock` no test can flip, so
/// every branch reading it off-default is one the mutation gate cannot reach. The cap is THREADED as
/// an argument into [`crate::core::tier_cap`] and graded there instead.
#[mutants::skip]
pub fn hold_per_tier_us() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_us)
            .unwrap_or(0)
    })
}

/// (#233) admit swap-based funds to the CORE lane. `#[mutants::skip]` for the reason every knob
/// reader here carries it: the value is a process-wide `OnceLock` no test can flip, so a branch
/// reading it off-default is one the mutation gate cannot reach. The flag is THREADED into
/// [`crate::core::hold_miss_leg_with`] and graded there instead — (#204)'s rule.
#[mutants::skip]
pub fn hold_allow_swap() -> bool {
    use std::sync::OnceLock;
    static ON: OnceLock<bool> = OnceLock::new();
    *ON.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_allow_swap)
            .unwrap_or(false)
    })
}

/// (#240) admit funds carrying no TER datum to the CORE lane. `#[mutants::skip]` for the same
/// reason as [`hold_allow_swap`]: a process-wide `OnceLock` no test can flip makes the off-default
/// branch unreachable for the mutation gate. The flag is THREADED into
/// [`crate::core::hold_miss_leg_with`] and graded there instead — (#204)'s rule.
#[mutants::skip]
pub fn hold_allow_unknown_ter() -> bool {
    use std::sync::OnceLock;
    static ON: OnceLock<bool> = OnceLock::new();
    *ON.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_allow_unknown_ter)
            .unwrap_or(false)
    })
}

/// (#233) the tier-3 swap TER credit, in percentage points; 0.0 = no credit. Same `#[mutants::skip]`
/// and the same reason as [`hold_allow_swap`] — it is threaded into the `hold_core_list` comparator,
/// which is where [`crate::core::net_ter`] is graded.
#[mutants::skip]
pub fn hold_us_swap_ter_credit() -> f64 {
    use std::sync::OnceLock;
    static CREDIT: OnceLock<f64> = OnceLock::new();
    *CREDIT.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_us_swap_ter_credit)
            .unwrap_or(0.0)
    })
}

/// (#235) The withholding cost, in percentage points, that the CORE sort charges a fund whose
/// domicile does NOT carry the US treaty rate — the number `picks::dom_rank` has always applied as an
/// unbounded bucket, now spent in the units [`crate::core::net_ter`] already speaks.
///
/// 0.0 is OFF and the shipped default: the bucket stays the sort key ahead of price, byte-identical.
/// Read ONCE outside the comparator and THREADED in, the same way (#233) threaded the swap credit —
/// a sort must not reach for a `OnceLock` once per comparison, and (#204) wants the branch gradeable.
/// Skipped for the same reason as [`hold_us_swap_ter_credit`] directly above: this body is a
/// `OnceLock` read no test can flip, so its three mutants are unkillable HERE. The decisions it
/// feeds live in `picks::dom_cost` and `picks::hold_core_cmp`, both of which take the value as an
/// ARGUMENT and are graded there — which is what (#204) actually asks for.
#[mutants::skip]
pub fn hold_domicile_penalty() -> f64 {
    use std::sync::OnceLock;
    static PENALTY: OnceLock<f64> = OnceLock::new();
    *PENALTY.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_domicile_penalty)
            .unwrap_or(0.0)
    })
}

/// (#230) the SMALL-CAP sleeve's own row cap; 0 = inherit [`hold_per_tier`]. Same `#[mutants::skip]`
/// as its three twins and for the same reason: the value is a process-wide `OnceLock` no test can
/// flip, so every branch reading it off-default is one the mutation gate cannot reach. The cap is
/// THREADED as an argument into [`crate::core::tier_cap`] and graded there instead.
#[mutants::skip]
pub fn hold_per_tier_size() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_size)
            .unwrap_or(0)
    })
}

/// (#228) the world FACTOR sleeve's own row cap; 0 = inherit [`hold_per_tier`]. Same `#[mutants::skip]`
/// as [`hold_per_tier_sector`] and [`hold_per_tier_country`], for the same reason: the value is a
/// process-wide `OnceLock` no test can flip, so every branch reading it off-default is one the
/// mutation gate cannot reach. The cap is THREADED as an argument and graded there instead.
#[mutants::skip]
pub fn hold_per_tier_factor() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_factor)
            .unwrap_or(0)
    })
}

/// (#237) the EUROPE sleeve's own row cap; 0 = inherit [`hold_per_tier`]. Same `#[mutants::skip]` as
/// [`hold_per_tier_sector`], for the same reason: the value is a process-wide `OnceLock` read once from
/// config, so the `--lib` suite cannot choose it and a mutant replacing this with whatever it ships at is
/// unkillable rather than merely unkilled. The DECISION is `core::tier_cap`, which takes the cap as an
/// ARGUMENT and is graded there — the threading (#204) established.
#[mutants::skip]
pub fn hold_per_tier_europe() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_europe)
            .unwrap_or(0)
    })
}

/// (#220) the world SECTOR sleeve's own row cap; 0 = inherit [`hold_per_tier`]. Free accessor for
/// the same reason as its two twins — `hold_core_list` takes no `tuning`, and the header line quotes
/// this number too, so one reader is what stops the printed cap and the applied cap drifting apart.
///
/// Skipped for [`hold_factor_sleeve`]'s reason verbatim: the answer is the AMBIENT config, which the
/// `--lib` suite cannot choose, so the mutant replacing this with whatever it currently ships at is
/// unkillable rather than merely unkilled. The DECISION is `core::tier_cap`, which takes the cap as
/// an argument and is graded there — the same threading (#204) used for `hold_size_sleeve_ter`.
/// Its two elder twins `hold_per_tier` and `hold_per_tier_all_world` carry no skip only because
/// neither has been inside a diff since it landed, so `--in-diff` has never re-graded them.
#[mutants::skip]
pub fn hold_per_tier_sector() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_sector)
            .unwrap_or(0)
    })
}

/// (#227) Same `#[mutants::skip]` as [`hold_per_tier_sector`], for the same reason: the value is a
/// process-wide `OnceLock` read once from config, so the `--lib` suite cannot choose it and a mutant
/// replacing this with whatever it ships at is unkillable rather than merely unkilled. The DECISIONS
/// this feeds are `core::tier_cap`, `core::sleeve_visible` and `core::country_sleeve_tier`, all
/// three of which take the cap as an ARGUMENT and are graded there.
#[mutants::skip]
pub fn hold_per_tier_country() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_country)
            .unwrap_or(0)
    })
}

/// (#239) Same `#[mutants::skip]` as [`hold_per_tier_country`], for the same reason: the value is a
/// process-wide `OnceLock` read once from config, so the `--lib` suite cannot choose it and a mutant
/// replacing this with whatever it ships at is unkillable rather than merely unkilled. The DECISIONS
/// this feeds are `core::tier_cap`, `core::sleeve_visible` and `core::nasdaq_sleeve_tier`, all three
/// of which take the cap as an ARGUMENT and are graded there.
#[mutants::skip]
pub fn hold_per_tier_nasdaq() -> usize {
    use std::sync::OnceLock;
    static CAP: OnceLock<usize> = OnceLock::new();
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier_nasdaq)
            .unwrap_or(0)
    })
}
...
pub fn hold_per_tier() -> usize {
    ...
    *CAP.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_per_tier)
            .unwrap_or(3)
    })
}

/// (#217) whether the world FACTOR sleeve exists. Free accessor for the same reason as the caps
/// below: `hold_core_list` takes no `tuning`. Deliberately the only config read on this path — the
/// decision is `core::factor_sleeve_tier`, which takes the flag as an argument and is graded there,
/// exactly as (#204) threaded `hold_size_sleeve_ter` for the size sleeve.
///
/// Skipped for [`hold_family_first`]'s reason verbatim: the answer is the AMBIENT config, which the
/// `--lib` suite cannot choose, so the mutant replacing this with whatever it currently ships at is
/// unkillable rather than merely unkilled.
#[mutants::skip]
pub fn hold_factor_sleeve() -> bool {
    use std::sync::OnceLock;
    static ON: OnceLock<bool> = OnceLock::new();
    *ON.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_factor_sleeve)
            .unwrap_or(false)
    })
}

/// (#218) whether the world SECTOR sleeve exists. Mirrors [`hold_factor_sleeve`] exactly, for its
/// reasons verbatim: free accessor because `hold_core_list` takes no `tuning`, and the only config
/// read on this path — the decision is `core::sector_sleeve_tier`, which takes the flag as an
/// argument and is graded there.
#[mutants::skip]
pub fn hold_sector_sleeve() -> bool {
    use std::sync::OnceLock;
    static ON: OnceLock<bool> = OnceLock::new();
    *ON.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_sector_sleeve)
            .unwrap_or(false)
    })
}

/// (#214) whether each CORE sleeve fills family-first. Free accessor for the same reason as the two
/// caps above: `hold_core_list` takes no `tuning`. It is deliberately the ONLY config read on this
/// path — the decision itself is `core::family_first_order`, which takes the flag as an argument
/// and is graded there.
///
/// Skipped for [`compute_threads`]'s reason, in its boolean form: the answer is the AMBIENT config,
/// which the `--lib` suite cannot choose (`OnceLock`, and `FOLIOMAN_CONFIG` is process-global), so
/// whichever of `true`/`false` this knob currently ships at, the mutant replacing it with that same
/// value is unkillable rather than merely unkilled. No `_of` split here because there is no decision
/// to extract — one field read, against `compute_threads_of`'s real `0`-means-auto rule.
#[mutants::skip]
pub fn hold_family_first() -> bool {
    use std::sync::OnceLock;
    static ON: OnceLock<bool> = OnceLock::new();
    *ON.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_family_first)
            .unwrap_or(false)
    })
}

/// (#272) free accessor twin of [`hold_family_first`], and free for the same reason its neighbours
...
pub fn hold_family_key_benchmark() -> bool {
    ...
    *ON.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.hold_family_key_benchmark)
            .unwrap_or(false)
    })
}
...
/// (#223) free accessor twin — `core::backtest_quote` fills `underwater_yrs` and has no `tuning`
/// handle, exactly as `vol_daily_equivalent` above. Unlike that one this flag is read on the BACKTEST
/// path only: the live daily path passes 252 explicitly at both `fetch.rs` call sites and reaches
/// `cadence == 252` here, so live behaviour is identical at either setting.
#[mutants::skip] // the answer is the ambient config, which the --lib suite cannot choose
pub fn underwater_monthly() -> bool {
    use std::sync::OnceLock;
    static FLAG: OnceLock<bool> = OnceLock::new();
    *FLAG.get_or_init(|| {
        merged_config()
            .and_then(|v| serde_yaml::from_value::<Settings>(v).ok())
            .map(|s| s.buy_heuristic.underwater_monthly)
            .unwrap_or(false)
    })
}
...
mod tests {
    ...
    fn validated_tilt_pin() {
        ...
        // (#147) 2026-08-28: the word "blind" came off this pin. (#1) set 0 because the walk-forward
        // could not see `pe_ratio`; (#147) filled it on the `fund` lane and MEASURED the 0.25/0.5/1.0
        // ladder at 12y and 8y. 0 STANDS, now on evidence: the tilt reorders the middle of the book and
        // never reaches the argmax, so rank-1 and the whole top-10 are byte-identical at every dose —
        // and still are at w=10.0, ten times the ladder's top rung. Read the (#147) receipt in
        // tests/ci-settings.yaml before moving this.
        assert_eq!(h.growth_value_weight, 0.0, "MEASURED and held at 0 by (#147) — {receipts}");
