...
pub struct BuyHeuristic {
    ...
    pub hold_factor_sleeve: bool, // (#217) turn on a TENTH CORE sleeve holding world FACTOR funds -- quality, value and equal weight -- which `core::NARROW` otherwise refuses on the token alone even though their geography is already world-broad. false = those three stay ordinary NARROW disqualifiers, which is today's behaviour byte-for-byte and the DEFAULT. A plain bool and NOT a TER ceiling, unlike `hold_size_sleeve_ter`: the live pond has three funds clearing the BASE 0.25% cap, one per factor family, so the sleeve needs no ceiling of its own and there is none to walk upward. IT OVERRIDES A SHIPPED PRINCIPLE BY DECISION, NOT BY EVIDENCE -- (#210)'s receipt says value/quality/momentum are "deliberately NOT in `core::SIZE`" because they re-weight names MSCI World already holds rather than adding an exposure. That judgement was never priced; this knob prices it. REVERT: set false, or revert on the (#210) principle alone -- no measurement is needed to justify turning it back off
