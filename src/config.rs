...
pub struct BuyHeuristic {
    ...
    pub print_selection_count: bool, // (#92) print, beside the two places that quote a SELECTED number, how many candidates it was selected from. `VERDICT_TOP: 3` — the basket the screen footer tells a reader to buy — is the argmax over `TOP_LADDER`'s 13 rungs, measured on the same data the report then quotes, with no best-of-N haircut and on the ~0.5-3.5 effectively independent windows (#91) counts. This file carries exactly one multiple-testing correction (the 14 fund factors, Šidák-tightened); the basket size has none. Display only — no gate, no score, no journal field. false = today's output, byte-identical, and the DEFAULT
    ...
    pub bootstrap_block_buckets: usize, // (#89) block length, in ~6-month cutoff buckets, for `bootstrap_edge_ci`. That fn calls itself a BLOCK bootstrap and resamples ONE bucket at a time, which is only sound if a bucket spans the dependence — it does not: two cutoffs of the same ticker six months apart share (2·years−1)/(2·years) of their forward path, 95.8% at 12y and 97.5% at 20y. A one-bucket block against that autocorrelation length reproduces the i.i.d. width, so the band is far too narrow, and that band is the load-bearing "straddles 0 -> noise" verdict every ship decision reads. The statistically-honest length is 4·years of buckets (buckets are 6mo, dependence spans 2·years) — 48 at a 12y hold, against the ~33 buckets that horizon HAS, which is the finding rather than a limit of this knob. 0 (and 1) = today's one-bucket blocks, byte-identical and the DEFAULT
...
impl Default for BuyHeuristic {
    fn default() -> Self {
        BuyHeuristic {
            ...
            growth_value_floor_pct: 0.0,   // (#75) off by default — at 0 both call sites skip the trim entirely, so the served table and every prior receipt's book are byte-identical. MEASURABLE on `backtest <h> universe fund` only; grade it on the top-3 book before shipping any value
            ...
            backtest_calendar_cadence: false, // (#117) OFF = the fixed 12/36/6 (and 252/750/126) every golden and every fitted receipt in ci-settings was measured under. Turning it on re-weights the SAMPLE — a daily-granularity name stops contributing ~40x the cutoffs of a monthly one — which moves every edge, rho and OOS number at once rather than moving one gate
            ...
            print_selection_count: false,  // (#92) OFF = today's output byte-for-byte, which is what every golden in tests/fixture pins. Display-only: turning it on adds a best-of-N caveat to the top-N table header and the screen footer, and changes no number
            ...
            bootstrap_block_buckets: 0,    // (#89) one bucket per draw = the band every receipt in this repo was read off. Raising it WIDENS every band and can only turn a "clears 0" into a "straddles 0", never the reverse — see the field comment and the ci-settings receipt
