...
impl Default for BuyHeuristic {
    fn default() -> Self {
        BuyHeuristic {
            ...
            long_trend_cap: 30.0,          // cap the long-leg CAGR at 30%/yr (a +46%/yr coin doesn't dwarf a +14%/yr one). CURVED at 12y (#3g): the ceiling wants to come DOWN, not up — 15/20/25/30 read edge +175.6/+185.1/+191.1/+181.3 and everything above falls monotonically (40 +177.3, 50 +168.1, 60 +168.1), confirming the older on-horizon 30->50 point-test. The +9.8 hump at 25 is inside the bootstrap band AND this knob is SHARED with the on-sale lane (`long_trend_weight × min(long_cagr, cap)`), so a growth-lane curve prices only half its effect — moving it needs the on-sale curve too. NOT moved
