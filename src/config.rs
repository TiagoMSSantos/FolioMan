...
pub struct BuyHeuristic {
    ...
    pub life_cagr_max_years: f64,    // (#3l) `use_life_cagr: false` window swap: >0 replaces the 20/8/5Y rung ladder with the endpoint CAGR over the last min(age, this) years — an 18y-old name is judged on 18 years instead of its 8Y rung (the 8->20 dead band), a 40y-old on the last N only. Whole-life mode (`use_life_cagr: true`) is untouched. Pool unchanged by construction: `core::capped_life_cagr` returns None under 5y of history (the rung ladder never ranked a name without a 5Y leg) and `score_parts` still requires a rung. The (#3i) whole-life reject bar keeps reading the UNCAPPED life CAGR — this knob moves the rank window, not the proven-compounder bar. Routed through `long_cagr_from`, so all seven readers move together (gate leg, trend, accel, sharpe, calmar, trend_health, PEG). 0 = off (rung ladder, today's behaviour). Edge-affecting -> graded on the stress grid before shipping any value here
...
impl Default for BuyHeuristic {
    fn default() -> Self {
        BuyHeuristic {
            ...
            life_cagr_max_years: 0.0,      // (#3l) OFF by default = the 20/8/5Y rung ladder; graded arm values are 20/30 (min(age,N)-window CAGR in false mode)
