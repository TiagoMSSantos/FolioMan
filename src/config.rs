...
pub struct BuyHeuristic {
    ...
    pub growth_min_cagr: f64,        // growth GATE: CAGR (%/yr) floor — below this it's not a proven compounder, just an expensive laggard. TWO legs, same number: (1) the long-leg CAGR (20/8/5Y rung) — validated, runs everywhere; (2) (#3i) the WHOLE-LIFE CAGR (`life_cagr`) — SCREEN-ONLY by construction, since backtest_quote leaves that field None, so it cannot fire in a backtest and the 12->14 receipt still measures leg (1) alone. Leg (2) only ever rejects, never admits, so the live pool is a strict SUBSET of the measured pool
