...
pub struct Sizing {
    ...
    pub max_name_pct: f64,   // (P5) no single ISSUER may exceed this % of gross — `(#262)` made that true, by collapsing twin listings in `size::first_per_issuer` before the weights are struck; it read per-TICKER until then, and an 8% Alphabet book printed as two 4% rows. Excess is redistributed to the UNCLAMPED members of the same class in proportion to their vol-target weight, iterated to a fixpoint (capped at 5 passes — the loop is monotone, so a pass that moves nothing ends it). 0 = off
...
impl Default for Sizing {
    fn default() -> Self {
        Self {
            ...
            budget_etf: 25.0,
            budget_crypto: 5.0,
            max_name_pct: 6.0, // (#262) was 4.0 while the cap was per-TICKER. Per ISSUER it binds on a book that already ran 8% Alphabet, so 6.0 both LOWERS the true single-issuer maximum and lifts deployed 37.0 -> 42.0; the revert is 4.0
            max_sector_pct: 25.0,
            spill_names: 3,
        }
    }
