...
pub struct BuyHeuristic {
    ...
    pub hold_min_aum_eur: f64, // (#267) the CORE admission AUM floor, in EUR, as a named number instead of the `1e9` literal buried in `core::hold_miss_leg_at` -- the LAST bare literal among the CORE admission bars, every other one (`hold_max_ter`, `hold_size_sleeve_ter`, `hold_allow_swap`, `hold_allow_unknown_ter`) having been named already. Naming it is byte-identical; the value below is not, and tests/ci-settings.yaml ships 5e8 -- the same Default/shipped split as `hold_ucits_or_domicile` and `hold_allow_swap`. MISSING AUM STILL PASSES: the leg reads `aum_shown()` inside `if let Some(a)`, which is (#224)'s fix for non-negotiable #5, and this knob does not touch it -- only a KNOWN sub-floor AUM refuses. The printed reject reason formats ITS FLOOR FROM THIS FIELD (non-negotiable #4), so the message can never quote a number the filter did not use -- (#102)'s rule when it named `hold_max_ter`. WHY IT MOVED, and it is a POLICY change taken BY DECISION rather than by evidence: (#224) and (#226) each refused lowering this floor, both on the test "none of these is a NEW index family for its sleeve" -- correct for a CAP-BOUND sleeve, but ex-US (tier 4) and Asia-Pacific (tier 7) are measured EMPTY BEHIND their caps, where a fund does not need a new family to print, it needs a free slot. The user took that decision explicitly after being shown the cost, the standing (#227) and (#239) both ship on. 1e9 = the literal that shipped, byte-identical, and the DEFAULT
...
impl Default for BuyHeuristic {
    fn default() -> Self {
        BuyHeuristic {
            ...
            hold_min_aum_eur: 1e9, // (#267) the literal that shipped inside `core::hold_miss_leg_at`, extracted verbatim. This is the CONFIG-LESS shape only: tests/ci-settings.yaml ships 5e8.
            ...
            hold_domicile_penalty: 0.0, // (#235) OFF = `dom_rank`'s bucket is still the sort key ahead of price, exactly as it shipped. Non-negotiable #1. tests/ci-settings.yaml ships this 0.20, the same Default/shipped split as `hold_allow_swap` and `hold_us_swap_ter_credit`.
