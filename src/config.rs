...
impl Default for BuyHeuristic {
    fn default() -> Self {
        BuyHeuristic {
            ...
            growth_value_floor_pct: 0.0,   // (#75) off by default — at 0 both call sites skip the trim entirely, so the served table and every prior receipt's book are byte-identical. MEASURABLE on `backtest <h> universe fund` only; grade it on the VERDICT_TOP book before shipping any value — that basket was 3 when (#75) was refused and is 10 since (#120), which is exactly the half of the refusal that no longer holds
