...
pub async fn run(args: Vec<String>) {
    ...
    for ticker in &tickers {
        ...
        let peg_cagr = q.and_then(|q| picks::peg_cagr_pct(q, &settings.buy_heuristic));
        let (block, has_table) =
            render_annual(ticker, source, &rows, today, close, peg_cagr, &settings.buy_heuristic);
        print!("{block}");
        if has_table {
            tables_printed += 1;
        }
...
fn render_annual(
    ...
    let peg = close_filer.and_then(|p| core::peg_yield(ff.eps_ttm, peg_cagr, p));
