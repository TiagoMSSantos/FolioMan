...
pub async fn run(args: Vec<String>) {
    ...
    for q in fetch::quotes(&client, &settings.urls, &fx, &tickers, settings.dip_days, settings.high_days, false, true, &settings.anchor_windows, None).await { // news on: alert body shows headlines; keys on price drop, not returns
